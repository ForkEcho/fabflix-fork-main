...
function handleLookup(query, doneCallback) {
    ...
    const cache = sessionStorage.getItem(query + "_searchSuggestion");
    if (cache) {
        doneCallback(JSON.parse(cache));
        return;
    }
    ...
    jQuery.ajax({
        ...
        url: "movie-suggestion?query=" + encodeURIComponent(query),
...
function handleLookupAjaxSuccess(data, query, doneCallback) {
    ...
    sessionStorage.setItem(query + "_searchSuggestion", JSON.stringify(data));
...
function handleNormalSearch(query) {
    ...
    window.location.assign("./movie-list.html?fulltext-title=" + encodeURIComponent(query));
}
...
let fullTextForm = $("#fulltext-input-div");

function submitFullTextSearch(formSubmissionEvent){
    console.log("Submitting Full Text Search...");
    formSubmissionEvent.preventDefault();
    const params = new URLSearchParams(fullTextForm.serialize());
    window.location.assign("./movie-list.html?" + params);
}

fullTextForm.submit(submitFullTextSearch)
